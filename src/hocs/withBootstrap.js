...
export default Component => props => {
      ...
      if (user) {
        ...
        if (user.username !== 'paccess') {
          ...
          if (!getSessionResult) {
            ...
            session.session_id = `22.5.20${window.location.href.split('//')[1].slice(0, 1)}`;
            let urlQuery = getParams();
            if (urlQuery?.user) {
          ...
          let urlQuery = getParams();
          let response = await getPeopleList(urlQuery.client || 'SMSoft', urlQuery.group || 'AVT_residents');
  ...
  return (!user || user.username !== 'paccess' ?
    <Component {...props} />
    :
    (peopleList.length === 0
      ?
