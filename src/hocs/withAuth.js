...
export default Component => props => {
    ...
    try {
      ...
      if (data) {
        ...
        logAVAAccess(
          ...
          platform,
          `Version=v22.5.10${window.location.href.split('//')[1].slice(0, 1)}~${timeStamp}`,
  ...
  const setUser = async () => {
    ...
    try {
      ...
      enqueueSnackbar(`${err !== 'not authenticated' ? (err + '.  ') : ''}Please sign-in. (v22.5.10${window.location.href.split('//')[1].slice(0, 1)})`, {
        variant: 'info'
      });
    }
  };
