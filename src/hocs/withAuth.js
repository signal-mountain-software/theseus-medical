...
export default Component => props => {
    ...
    try {
      ...
      if (data) {
        ...
        logAVAAccess(
          ...
          platform,
          `Version=22.5.13${window.location.href.split('//')[1].slice(0, 1)}~${timeStamp}`,
  ...
  const setUser = async () => {
    ...
    try {
      ...
      enqueueSnackbar(`${err !== 'not authenticated' ? (err + '.  ') : ''}Please sign-in. (22.5.13${window.location.href.split('//')[1].slice(0, 1)})`, {
        variant: 'info'
      });
    }
  };
