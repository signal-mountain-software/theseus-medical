    ...
    try {
      ...
      if (data) {
        ...
        logAVAAccess(
          ...
          `Version=22.11.11${window.location.href.split('//')[1].slice(0, 1).toUpperCase()}~${timeStamp}`,
  ...
  const setUser = async () => {
    ...
    try {
      ...
      if (cookieValues.user_id) {
        ...
        enqueueSnackbar(`${err !== 'not authenticated' ? (err + '.  ') : ''}Please sign-in. (AVA version 22.11.11${window.location.href.split('//')[1].slice(0, 1).toUpperCase()})`, {
          variant: 'info'
        });
      }
    }
