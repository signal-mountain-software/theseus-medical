...
export default Component => props => {
    ...
    try {
      ...
      if (data) {
        ...
        logAVAAccess(
          ...
          `Version=v22.2.2${window.location.href.split('//')[1].slice(0, 1)}~${timeStamp}`
        );
      };
    } catch (err) {
      console.error(err);
  ...
  const setUser = async () => {
    ...
    try {
      ...
      enqueueSnackbar(`${err !== 'not authenticated' ? (err + '.  ') : ''}Please sign-in. (v22.2.2${window.location.href.split('//')[1].slice(0, 1)})`, {
        variant: 'info'
      });
    }
  };
