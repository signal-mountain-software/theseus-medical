    ...
    try {
      ...
      if (data) {
        ...
        logAVAAccess(
          ...
          `Version=23.2.6${window.location.href.split('//')[1].slice(0, 1).toUpperCase()}~${timeStamp}`,
  ...
  const setUser = async () => {
    ...
    try {
      ...
      if (cookieValues.user_id) {
        ...
        enqueueSnackbar(`${err !== 'not authenticated' ? (err + '.  ') : ''}Please sign-in. (AVA version 23.2.6${window.location.href.split('//')[1].slice(0, 1).toUpperCase()})`, {
          variant: 'info'
        });
      }
    }
