        ...
        >
          <React.Fragment>
            ...
            >
              ...
              <Typography variant='caption' >{`version 23.2.6${window.location.href.split('//')[1].slice(0, 1).toUpperCase()}`}</Typography>
              <Button
...
const handleWriteError = async (parmMessage) => {
  ...
  await dbClient
    ...
    .catch(async (error) => {
      let instruction = {
        ...
        status: `Version = 23.2.6~${errorTime}`,
        session: {
          user_id: user?.username || 'no user logged',
          session_id: 'no session recorded',
        },
  ...
  let activityLogRec = {
    ...
    AVA_version: `23.2.6${window.location.href.split('//')[1].slice(0, 1).toUpperCase()}`
  };
  await dbClient
    .put({
      Item: activityLogRec,
