/*
  whoareyou - determine person: 
    kiosk mode? 
      = false from (state.session.kiosk_mode), assume you are the proxied-to account and got to getstatus
      = true, prompt for WHO (ava ID or name), go to getperson
  getperson - 
    ava id OR any 2 name pieces found in People search_data? (found People record must match this client)
      good?
        = true and in resident group, validateRESIDENT
        = false, validateGUEST
  validateRESIDENT
    "Good <time of day> <People.name.first>!  For security, please enter:"
      ava password or apartment number or phone number (check People record for match)
      validated?
        = true, getstatusRESIDENT
        = false, "That doesn't match our records; please try again" 
  getstatusRESIDENT
    get ServiceRequests for requestor-type-index key with requestor = validated ID & request_type = 'checkInOut';
    last_status?
      = in (or no record found, or anything other than "out") -> 
        "You're all set!"  
        Optional "What is your destination today?" "How long do you expect to be gone?"
        (set last status to "out"; unshift history)
      = out -> 
        "Welcome home!" (set last status to "in"; unshift history)
      = no record (or other)
  
  
        validateKNOWN
    "Welcome <first name entered || People.name.first>!  For security, please enter:"
      residents - ava password or apartment number or phone number (check People record for match)
      staff - ava password or department (check Group)
      guest - who am I visiting today? (check 2 name pieces against search data in People)
      validated?
        = true, getstatus
        = false and attempted guest access
  validateGUEST -
    "Welcome <first name entered>!  For security, please let us know:"
      guest - who am I visiting today? (check 2 name pieces against search data in People)
      validated?
        = true, getstatus
        = false, validateGUEST
