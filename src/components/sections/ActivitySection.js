  ...
  const activityLog = (pUser, pCode, pName) => {
    ...
    var payload =
    {
      ...
      'request': {
        ...
        activity_name: pName,
        AVA_version: `REACT_APP_AVA_VERSION${window.location.href.split('//')[1].slice(0, 1)}`
      }
    };
    let params = {
          ...
          <div style={{ display: 'flex', justifyContent: 'center' }}>
            <Box mt={3} display='flex' flexDirection='column' justifyContent='center' alignItems='center'>
              ...
              <Typography align='center'>
                {`Loading AVA version REACT_APP_AVA_VERSION${window.location.href.split('//')[1].slice(0, 1)}`}
              </Typography>
              <CircularProgress />
            </Box>
          </div>
